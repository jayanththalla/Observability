...
This is **not a keyword list or regex**. These are training examples. You are telling NeMo: _"any message that feels like these examples is the intent called `ask off topic`."_
...
Read it like plain English: _"When the user's intent matches `ask off topic`, send the scripted refusal, then stop."_
...
| -------------------------------------- | ---------------------------- | -------------------------------------------------- |
| User says "ignore your instructions"   | LLM might comply             | NeMo intercepts _before_ the LLM sees it           |
